...
// ... (Icon components tetap sama seperti kode Anda) ...
...
interface MeetingAttendanceModalProps {
  ...
  onClose: () => void;
  // PERBAIKAN: Ubah return type menjadi Promise agar bisa ditunggu (await)
  onMarkAttendance: (meetingId: string, memberId: string, signatureDataUrl: string) => Promise<{ success: boolean, memberName?: string, message?: string }>;
}
...
const MeetingAttendanceModal: React.FC<MeetingAttendanceModalProps> = ({ meeting, onClose, onMarkAttendance }) => {
  ...
  const [isSubmitting, setIsSubmitting] = useState(false); // State baru untuk loading simpan
  ...
  // PERBAIKAN: Fungsi ini sekarang ASYNC untuk menunggu database
  const handleSignatureComplete = async (signatureData: string) => {
      if (!scannedMemberId || !signatureData || isSubmitting) return;

      setIsSubmitting(true);
      setErrorMessage('');

      try {
          // Memanggil fungsi dari DataContext yang melakukan fetch ke database
          const result = await onMarkAttendance(meeting.id, scannedMemberId, signatureData);
          
          if (result.success) {
              setScannedMemberName(result.memberName || 'Anggota');
              setStep('success');
          } else {
              setErrorMessage(result.message || 'Gagal menyimpan absensi ke database.');
              // Jika gagal, kembali ke scan agar bisa coba lagi atau ganti user
              setStep('scan');
          }
      } catch (err) {
  ...
  const styles = `
    ...
    .reader-container video { object-fit: cover; border-radius: 1rem; }
  `;
    ...
    <Modal isOpen={true} onClose={onClose} maxWidth="max-w-5xl">
        ...
        <div className="h-[85vh] flex flex-col bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100 overflow-hidden relative">
            ...
            <div className="px-6 py-4 border-b border-gray-100 dark:border-gray-800 flex justify-between items-center bg-gray-50/50 dark:bg-gray-900/50 backdrop-blur-sm shrink-0">
                <div>
                    <h2 className="text-xl font-bold text-gray-900 dark:text-white leading-tight mb-1">{meeting.title}</h2>
                    <div className="flex items-center gap-3 text-xs text-gray-500 font-medium">
                        <span className="bg-gray-100 dark:bg-gray-800 px-2 py-0.5 rounded">{meetingDate.toLocaleDateString('id-ID', { day: 'numeric', month: 'short', year: 'numeric' })}</span>
                        <span className="bg-gray-100 dark:bg-gray-800 px-2 py-0.5 rounded">{meeting.location}</span>
                    </div>
                </div>
            ...
            <div className="flex-grow relative flex overflow-hidden">
                ...
                {step === 'scan' && (
                    <div className="w-full h-full flex flex-col md:flex-row p-6 gap-6">
                        <div className="flex-1 relative bg-black rounded-3xl overflow-hidden shadow-inner border border-gray-800">
                            ...
                            {errorMessage && (
                                <div className="absolute bottom-4 left-4 right-4 z-40">
                                    <div className="bg-red-500 text-white px-4 py-3 rounded-xl text-sm font-medium shadow-lg text-center">
                                        {errorMessage}
                                    </div>
                                </div>
                            )}
                        ...
                        <div className="w-full md:w-80 flex flex-col gap-4">
                            <div className="bg-gray-50 dark:bg-gray-800/50 p-5 rounded-3xl border border-gray-100 dark:border-gray-800 shadow-sm">
                                <h3 className="font-bold text-sm mb-4 flex items-center gap-2"><KeyboardIcon className="w-4 h-4 text-emerald-600" /> Input Manual</h3>
                                <form onSubmit={handleManualSubmit}>
                                    <input 
                                        type="text" 
                                        placeholder="Ketik NIDN..." 
                                        className="w-full bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl px-4 py-3 text-sm outline-none"
                                        value={manualId}
                                        onChange={e => setManualId(e.target.value)}
                                    />
                                    <button type="submit" className="w-full mt-3 bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-3 rounded-xl text-sm shadow-lg">Lanjut</button>
                                </form>
                            </div>
                            ...
                            <div className="flex-grow bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-3xl p-5 overflow-y-auto">
                                <h3 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-3">Contoh ID</h3>
                                <div className="space-y-2">
                                    {/* Gunakan ID asli dari database Anda untuk simulasi ini agar masuk ke DB */}
                                    <button onClick={() => simulateScan('123456')} className="w-full p-3 rounded-xl bg-gray-50 hover:bg-gray-100 dark:bg-gray-700/30 text-left flex items-center gap-3">
                                        <div className="w-8 h-8 rounded-full bg-blue-100 flex items-center justify-center text-blue-600"><UserIcon className="w-4 h-4"/></div>
                                        <div><p className="text-xs font-bold">Simulasi NIDN</p><p className="text-[10px] text-gray-500">NIDN: 123456</p></div>
                                    </button>
                                </div>
                            </div>
                            <button onClick={onClose} className="py-3 text-sm font-medium text-gray-500">Batal</button>
                        </div>
                    </div>
                )}
                ...
                {step === 'sign' && (
                    <div className="w-full h-full flex flex-col items-center justify-center p-8">
                        ...
                        <p className="text-gray-500 text-sm mb-6 text-center">Bubuhkan tanda tangan Anda di bawah ini.</p>
                        ...
                        <div className="relative bg-white dark:bg-gray-800 rounded-xl overflow-hidden shadow-2xl border border-gray-100 dark:border-gray-700">
                            ...
                            />
                            {isSubmitting && (
                                <div className="absolute inset-0 bg-white/60 dark:bg-gray-900/60 flex items-center justify-center z-50">
                                    <div className="flex flex-col items-center gap-2">
                                        <div className="w-8 h-8 border-4 border-emerald-500 border-t-transparent rounded-full animate-spin"></div>
                                        <p className="text-xs font-bold text-emerald-600">Menyimpan ke Database...</p>
                                    </div>
                                </div>
                            )}
                        </div>
                        ...
                        <button 
                            ...
                            className="mt-8 text-sm font-medium text-gray-400 hover:text-gray-600 flex items-center gap-2"
                        >
                ...
                {/* 3. STEP: SUCCESS */}
                {step === 'success' && (
                    <div className="w-full h-full flex flex-col items-center justify-center p-8 text-center animate-in slide-in-from-bottom-10">
                        <div className="relative mb-6">
                            <div className="bg-white dark:bg-gray-800 p-4 rounded-full shadow-lg border border-gray-100 dark:border-gray-700">
                                <CheckIcon className="w-16 h-16 text-green-500" />
                            </div>
                        </div>
                        <h3 className="text-3xl font-black text-gray-900 dark:text-white">Terima Kasih!</h3>
                        <div className="mt-6 mb-8 bg-gray-50 dark:bg-gray-800/50 px-8 py-6 rounded-3xl border border-gray-100 dark:border-gray-700 max-w-sm w-full">
                            ...
                            <p className="text-2xl font-bold text-emerald-600">{scannedMemberName}</p>
                            <div className="h-px w-full bg-gray-200 dark:bg-gray-700 my-4"></div>
                            <p className="text-xs text-gray-500">Absensi Anda telah tercatat secara otomatis.</p>
                        </div>
                        <button onClick={resetProcess} className="bg-gray-900 dark:bg-white text-white dark:text-gray-900 font-bold py-3 px-8 rounded-xl">Lanjut Scan Berikutnya</button>
                    </div>
                )}
            </div>
        </div>

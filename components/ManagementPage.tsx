...
// --- INLINE ICONS (Visual Enhancement) ---
...
const ManagementPage: React.FC<Omit<ManagementPageProps, 'members' | 'meetings'>> = ({ 
  ...
  const filteredMembers = useMemo(() => {
    // Pastikan members adalah array sebelum melakukan filter
    if (!Array.isArray(members)) return [];
    
    return members.filter(m => 
        m.fullName.toLowerCase().includes(memberSearchTerm.toLowerCase()) || 
        m.nidn.includes(memberSearchTerm) || 
        m.position.toLowerCase().includes(memberSearchTerm.toLowerCase())
    );
}, [members, memberSearchTerm]);

  const filteredMeetings = useMemo(() => meetings?.filter(m => 
          m.title.toLowerCase().includes(meetingSearchTerm.toLowerCase()) || m.location.toLowerCase().includes(meetingSearchTerm.toLowerCase())
  ) || [], [meetings, meetingSearchTerm]);

  // --- HANDLERS (SUDAH DIPERBAIKI) ---
  ...
  const handleDuplicateMeeting = (id: string) => {
      if(duplicateManagementMeeting) duplicateManagementMeeting(id);
  };

  const handleDeleteMeeting = (id: string) => {
      if(window.confirm('Apakah Anda yakin ingin menghapus agenda rapat ini?')) {
          deleteManagementMeeting(id);
      }
  };
  ...
  const avgAttendance = totalMeetings > 0 ? Math.round((meetings?.reduce((acc, m) => acc + m.attendees.length, 0) || 0) / totalMeetings) : 0;
